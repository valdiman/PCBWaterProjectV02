...
PS_data <- PS_data %>%
  select(SYS_SAMPLE_CODE, SAMPLE_DATE, SAMPLE_TYPE_CODE, MATRIX_CODE, ANALYTIC_METHOD,
...
transposed_data <- PS_data %>%
  pivot_wider(
    id_cols = c(SYS_SAMPLE_CODE, SAMPLE_DATE, SAMPLE_TYPE_CODE, MATRIX_CODE,
...
grouped_data <- PS_data %>%
  distinct(SYS_SAMPLE_CODE, SAMPLE_DATE, SAMPLE_TYPE_CODE, MATRIX_CODE,
           ANALYTIC_METHOD, LATITUDE, LONGITUDE, RESULT_UNIT)
